...
[2026-05-13] | `quorum_state` blackboard: single jsonb-heavy row per quorum with CAS on `version` (checklist 11.6) | The Magentic-One orchestrator (PR #14) had no shared "what the quorum believes right now" surface — speaker selection was driven only by recent contributions, and authority arbitration (11.7) is impossible without a "current proposal" object to override and an "open decision" object to dissent against. The blackboard is one row holding five jsonb lists (open_questions, proposals, decisions, conflicts, dissents) plus a `version` int; agents read the row, mutate the relevant list in Python, write back with `.eq("version", v)`. Read access is open (projector subscribes via realtime); writes are service-role only. Routes are GET-only — agents mutate via the `quorum_state` helper module, not HTTP. The pre-existing `GET /quorums/{id}/state` route was unrelated (returns live quorum / contributions / artifact); we surfaced the blackboard at `GET /quorums/{id}/blackboard` to avoid colliding with that contract. | One-row-per-fact normalised schema (rejected — over-engineered for the access pattern; agents need atomic snapshots of all open proposals/decisions every round and would otherwise pay N queries per turn), embed in `artifacts.sections` (rejected — conflates draft-output structure with in-flight deliberation state, and the projector needs to read these independently), event-sourced append-only log (rejected — replay cost for every read; the blackboard is the projection, not the source of truth — contributions already serve that role)
