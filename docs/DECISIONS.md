...
[2026-05-13] | Wrap sync Supabase `.execute()` calls in `asyncio.to_thread` via the `apps/api/db/aexec.py` helper on autonomy hot paths (checklist 12.6) | The `supabase-py` client is synchronous — every `.execute()` blocks the calling thread. Inside FastAPI's async loop that stalls every other coroutine until the DB responds, which under heavy autonomy load (4+ quorums × autonomy 1.0) shows up as WebSocket disconnects, missed heartbeats, and visible UI freezes. The new `await aexec(builder)` helper pushes each call onto the default thread-pool executor so the loop keeps spinning. Sync helpers used in BOTH sync and async contexts (e.g. `_maybe_auto_contribute`, `_load_role_and_config`, `lookup_endpoint`) stay sync; the async callers bounce them through `aexec(thunk)` to keep the boundary clean. Files converted: `agent_engine.py`, `autonomy_loop.py`, `agents/orchestrator.py`, `coordination/supabase_backend.py`, `quorum_a2a/a2a_server.py`, `quorum_a2a/a2a_client.py`. Route handlers in `routes.py` that aren't on the autonomy hot path are deferred (user-triggered, sequential, no stall risk). | Approach B: migrate to `supabase-py-async` / `supabase.AsyncClient` (rejected for this PR — much wider blast radius, API differs slightly, higher pre-expo risk; documented as post-expo follow-up), do nothing (rejected — the audit flagged this as the last big refactor of the night)
