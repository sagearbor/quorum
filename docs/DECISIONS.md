...
[2026-05-13] | FacilitatorAgent (9.4) uses an MCPToolAdapter bridge instead of `pydantic_ai.Agent(tools=[...])` for the MCP servers from 11.2 | The repo's hard rule (CLAUDE.md) is that every LLM call routes through the `LLMProvider` factory so `QUORUM_TEST_MODE=true` swaps in `MockLLMProvider`. `Agent(tools=[...])` couples the tool runtime to Pydantic AI's Model/Provider, which would bypass the factory. The adapter keeps the project's invariant: tools are invoked deterministically via FastMCP's in-process `call_tool()`, results are folded into the prompt, and the LLM produces a typed `FacilitatorObservation`. The adapter also gives tests a `.calls` attribute for observability without needing a real MCP transport. | `pydantic_ai.Agent(tools=[...])` direct integration (rejected — breaks the LLMProvider rule and the test-mode swap), reimplementing the tools as plain async functions (rejected — duplicates 11.2 and loses the schema/ToolError contract), stdio MCP transport (rejected — needless plumbing for a single-process backend)

[2026-05-13] | `facilitator_observation` is a NEW WS frame type, not a replacement for `facilitator_narration` | The per-turn `facilitator_narration` (PR #14 / 11.3) is the avatar's spoken-aloud sentence picked by the orchestrator. The new `facilitator_observation` is meta-narration about the deliberation as a whole — fires every Nth round (default 3) and gets rendered as an ambient strip below the avatar, not spoken. Keeping them distinct frames lets the audience see both ("avatar speaks the turn" + "narrator notes the pattern") without one fighting the other for the same UI surface. | One unified `facilitator` frame with a `kind` discriminator (rejected — the consumers are different: TTS for narration, visible strip for observation, and merging the frames would couple their cadence), reuse `facilitator_narration` with a flag (rejected — would mean the avatar reads the meta-observation aloud, which the spec explicitly forbids)
